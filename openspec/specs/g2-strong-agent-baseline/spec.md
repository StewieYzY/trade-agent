...
#### Scenario: Explicit percentage presentation of a decimal ratio
- **WHEN** an AgentOutput metric explicitly includes `%` or `％`, preserves the source sign, and the same value is present in a decimal-stored field whose leaf-name token explicitly denotes ratio/rate/margin/ROE/share/percentage
- **THEN** the existing M0.2 grounding boundary accepts the unit-equivalent value

#### Scenario: No implicit scaling for unlabeled or unrelated numbers
- **WHEN** an AgentOutput number has no explicit percentage label, has the opposite sign, or the only similar input value belongs to a non-ratio field such as price/value/count/percentile
- **THEN** the grounding boundary does not infer a `100x` conversion and retains fabricated-number rejection
