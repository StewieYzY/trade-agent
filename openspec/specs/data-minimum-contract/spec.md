# data-minimum-contract Specification

## Purpose

定义 G1 快筛闭环的数据最小契约能力边界：G1 真正需要哪些数据、每个数据缺失时系统该 error/skip/degraded 还是提示人工、G1·G2·G3 数据边界如何划分、如何在不删除现有数据获取实现的前提下为后续 provider repair 留出接口。该 capability 只定义数据契约语义，不承载 runtime 实现（runtime 改动拆独立 implementation/repair child）。承接 G1-4「300 样本规模预检」暴露的「数据能力与缺失语义未冻结」缺口（5533→18、行业全未分类、industry_mapper 静默空 dict、risk 单源质押），为后续 `g1-4-data-source-resilience` repair child 提供契约依据。约束 AD-10（G1 数据能力未冻结前不 archive G1-4、不勾 umbrella 4.1/4.2、不开 G2 runtime）。
