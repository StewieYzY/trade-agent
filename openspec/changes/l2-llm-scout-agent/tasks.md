...
- [ ] 1.2 Implement `scout/prompt.py` with `SCOUT_SYSTEM_PROMPT` constant (5 questions + JSON schema from design.md §2)
- [ ] 1.3 Implement `scout/prompt.py` `format_snapshot(features: dict) -> str` function to format ~200-token user message (design.md §2 User Message template)
...
- [ ] 1.5 Add missing data handling in `assemble_snapshot`: use `None` placeholders and annotate "数据缺失" for missing fields
...
- [ ] 2.2 Implement `scout/parse.py` `apply_buffer_zone(verdict: str, confidence: int) -> tuple[str, bool]` function: confidence ≥ 60 → trust verdict; 40-60 → force "watch"; < 40 → force "watch" + flag anomaly
...
- [ ] 2.4 Add field validation in `parse_scout_output`: check verdict ∈ {deep_dive, watch, skip}, confidence ∈ [0, 100], flags are lists of strings
...
- [ ] 3.7 Filter output in `scout_batch`: return only candidates with `verdict == "deep_dive"`, sorted by confidence descending, capped at top 20 (AD-03 cost gate 200→20)
...
- [ ] 6.1 Create `tests/test_scout_prompt.py`: test `format_snapshot` with sample features dict, verify output matches design.md §2 template
...
- [ ] 7.1 Add docstring to `scout/prompt.py` explaining Scout prompt design (reference design.md §2 and total-design §5.2)
- [ ] 7.2 Add docstring to `scout/input_assembly.py` explaining L1→L2 data handoff (reference design.md §1 Decision 1), field sources (pe_ttm from valuation), and fin_models reuse
- [ ] 7.3 Add docstring to `scout/batch.py` explaining concurrent LLM calling strategy (reference design.md §4), top-20 cap (AD-03), and retry logic (1 retry after 2s)
- [ ] 7.4 Add docstring to `scout/parse.py` explaining buffer zone logic (reference design.md §3.2 and total-design §5.6)
- [ ] 7.5 Add docstring to `scout/quality.py` explaining 24h cache with input snapshot and `{date}` subdirectory (reference design.md §3.3)
- [ ] 7.6 Update `cli.py` help text for `scout` subcommand: explain usage, environment variables (LLM_API_KEY/BASE/MODEL, all required), and cost (~¥0.01/stock, top-20 cap)
...
- [ ] 8.3 Add cost estimation note to `README.md`: ~¥2/run for 200 stocks with 80% cache hit rate, top-20 cap ensures L3 cost stays within AD-03 budget
