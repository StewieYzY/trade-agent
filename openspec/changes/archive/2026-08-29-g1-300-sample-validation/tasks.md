...
- [x] 1.5 先写 identity/provenance 红测：canonical ticker、`run_id`、`profile_version`、调用方提供且经校验的 `input_ticker_set_hash`、`as_of`、schema version 和 fixture/reference 标识必须存在且一致；测试中断言不发生 provider/LLM 调用。

## 2. Minimal offline implementation

- [x] 2.1 新增 `value-screener/data/lib/validation_sample.py` 标准库 selector/contract 模块，显式接收 spot records、industry mapping、seed/config、运行元数据、`input_ticker_set_hash` 和 fixture provenance，不读取 `BasicFetcher`、全局缓存或任何 provider。
...
- [x] 4.2 运行适用的 `openspec change validate g1-300-sample-validation --strict`（当前 CLI 对同名 change/spec 需要显式 `change` 类型），确认 proposal/design/spec/tasks 合同完整。
- [x] 4.3 本 child 未运行 AkShare/东财/其他 provider/LLM，未执行真实 300+ 样本，未勾选 G1 umbrella 4.1/4.2，未生成 G1 evidence bundle；真实 G1 Gate 仍由 live evidence、全市场运行与用户复核等条件决定。
