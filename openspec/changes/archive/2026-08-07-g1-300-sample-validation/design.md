...
新增一个位于 `value-screener/data/lib/` 的标准库 selector/contract 模块。公共入口接收已经解析为 Python mapping 的 spot records、industry mapping 和显式运行元数据，返回 sample records 与 design summary。selector 不 import AkShare、不访问 `BasicFetcher`、不读取全局缓存。
...
`sample_size < 300` 时 summary 的 `full_market_eligible` 必须为 false。该输出不得被命名或写入 live evidence bundle。
