...
def _fmt(value, suffix: str = "", missing: str = "数据缺失") -> str:
    ...
    float 处理：整数浮点 → int 表示；非整数 → 最多 2 位小数去尾部 0。
    """
    if value is None:
        return missing
...
def format_snapshot(features: dict) -> str:
    ...
    # roe_3y 特殊处理：list → "28.5%, 25.3%, 22.1%"
    roe_3y = features.get("roe_3y")
    if isinstance(roe_3y, list) and roe_3y:
        roe_str = ", ".join(f"{v:.1f}%" for v in roe_3y)
    else:
        roe_str = "数据缺失"
    ...
    lines = [
        ...
        f"ROE(近3年): {roe_str}  ← {_fmt(roe_trend, missing='数据缺失')}",
        ...
        f"审计意见: {_fmt(features.get('audit_opinion'))}",
        f"近60日涨幅: {pct(features.get('price_change_60d'))}",
        f"换手率分位: {pct(features.get('turnover_avg_percentile_60d'))}",
